...
<xsl:stylesheet version="1.0"
 ...
 <xsl:choose>
 <xsl:when test="substring($controlField008,22,1)!='m'">Ressources continue</xsl:when>
 ...
 <xsl:when test="$leader6='m'">Fichiers informatiques</xsl:when>
 <xsl:when test="$leader6='e' or $leader6='f'">Carte</xsl:when>
 <xsl:when test="$leader6='g' or $leader6='k' or $leader6='r'">Matériels visuels</xsl:when>
 ...
 <xsl:otherwise>
 <xsl:text>Cliquer ici pour l'accès en ligne</xsl:text>
 </xsl:otherwise>
 </xsl:choose>
 </xsl:when>
 </xsl:choose>
 ...
 <xsl:when test="@ind2=6">
 <span class="label">Eclaté en .. et ...&nbsp;:</span>
 </xsl:when>
 <xsl:when test="@ind2=7">
 ...
 <a><xsl:attribute name="href">http://<xsl:value-of select="$OPACBaseurl"/>/cgi-bin/koha/opac-detail.pl?biblionumber=<xsl:value-of select="marc:datafield[@tag=999]/marc:subfield[@code='c']"/></xsl:attribute><xsl:attribute name="target">_blank</xsl:attribute>Ouvrir dans une nouvelle fenêtre</a>.
 </span>
 </xsl:if>
